...
if (!$conn) {
    die("<h3 style='color:red;'>❌ Failed to connect to database.</h3>");
}

$result = pg_query($conn, "SELECT id, username, email, created_at FROM ecommerce_users ORDER BY id DESC");
if (!$result) {
    die("<h3 style='color:red;'>Error fetching data: " . pg_last_error($conn) . "</h3>");
}
